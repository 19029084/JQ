...
@Slf4j
public class JQResponseBodyHandlerAdvice implements ResponseBodyAdvice {
    ...
    public Object beforeBodyWrite(Object body, MethodParameter returnType, MediaType selectedContentType, Class selectedConverterType, ServerHttpRequest request, ServerHttpResponse response) {
        if(MediaType.APPLICATION_JSON.equals(selectedContentType) || MediaType.APPLICATION_JSON_UTF8.equals(selectedContentType)){ // 判断响应的Content-Type为JSON格式的body
            if(body instanceof JQResponse){ // 如果响应返回的对象为统一响应体，则直接返回body
                return body;
            }else{
                // 只有正常返回的结果才会进入这个判断流程，所以返回正常成功的状态码
                JQResponse jqResponse =new JQResponse(JQResponseCode.SUCCESS.getCode(),JQResponseCode.SUCCESS.getMsg(),body);
                return jqResponse;
            }
        }
        // 非JSON格式body直接返回即可
